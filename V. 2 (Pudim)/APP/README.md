...
- **Análise Econômmica e Patrimonial**: Cálculo do índice FinScore baseado em demonstrações contábeis
...
# Ubuntu/Debian
sudo apt-get update
sudo apt-get install -y python3.12 python3.12-venv unzip
...
```bash
cd /opt/finscore
sudo -u finscore python3.12 -m venv .venv
...
deactivate
# Incluir a pasta wheels/ no arquivo ZIP
```
...
- **Erro venv/ensurepip**: Execute `sudo apt-get install -y python3.12-venv`.
...
- **Desenvolvimento**: FV
- **Algoritmos**: FV, com auxílio de Elpídio Neto (Assertif)
- **Design**: FV

---

*FinScore - Transformando dados contábeis em inteligência financeira*
Versão 2.1.00 (Pudim) (31/08/2026)
